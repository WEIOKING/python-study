...
# 定义默认参数要牢记一点：默认参数必须指向不变对象！不能指定默认参数为数组等，否则在函数中向数组添加、删除等操作会被保存，使默认参数发生改变
...
xn, yn = move(2, 3, 3)
print("nx, ny:", xn, yn)
# Python的函数返回多值是返回一个tuple，在语法上，返回一个tuple可以省略括号，而多个变量可以同时接收一个tuple，按位置赋给对应的值，所以，Python的函数返回多值其实就是返回一个tuple
t = move(2, 3, 3, 2)
